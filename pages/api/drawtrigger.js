// pages/api/drawTrigger.js
import fetch from 'node-fetch';

const PAGE_ID = process.env.PAGE_ID;
const ACCESS_TOKEN = process.env.FB_ACCESS_TOKEN;
const MAKE_TRIGGER_WEBHOOK = process.env.MAKE_TRIGGER_WEBHOOK; // 场景1的Webhook
...
export default async function handler(req, res) {
  try {
    ...
    // 发送 POST 请求到 Make 场景 1 的 Webhook，传送 postId
    ...
    const result = await makeRes.text(); // 可根据需要改为 json()
    ...
    return res.status(500).json({ success: false, message: '❌ 触发失败', error: err.message });
  }
}
