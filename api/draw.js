  ...
  try {
    ...
    if (alreadyDrawn && !DEBUG) {
      return res.status(403).json({
        error: '⚠️ 本场已抽奖一次，是否确认再次抽奖？加入 ?debug 可重新抽奖'
      });
    }
    ...
    for (const comment of allComments) {
      ...
      validEntries.push({
        ...
        from: {
          id: userId || `anon-${comment.id}`,
          name: userName || `留言 ${number}`
        },
        number,
        message: msg,
      });
    ...
    if (validEntries.length < 3) {
      return res.status(400).json({ error: '有效留言不足 3 条（需包含号码、访客、非主页）', total: validEntries.length });
    }
